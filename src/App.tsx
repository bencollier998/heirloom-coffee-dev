...
const OurStoryModal = ({ onClose }: { onClose: () => void }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
      ...
      <h2 className="text-4xl font-serif font-black italic text-brand-brown mb-6">Rooted in Craft, Grown with Love</h2>
      <p className="text-brand-brown/70 text-sm leading-relaxed mb-4">Heirloom Coffee was born in 2012 from a simple belief — that a great cup of coffee can make an ordinary moment extraordinary. What started as a single small café has grown into a beloved community staple, guided by the same values we started with.</p>
      <p className="text-brand-brown/70 text-sm leading-relaxed mb-8">We source our beans directly from small family farms, roast in small batches, and pair every cup with seasonal food made from scratch. Every detail is intentional. Every ingredient is chosen with care.</p>
          ...
          {products.map((p) => (
            <div key={p.name} className="bg-brand-cream-light rounded-2xl p-6 flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between mt-auto pt-3 border-t border-brand-brown/10">
                ...
                <button className="bg-brand-brown text-white px-4 py-2 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-black transition">Add to Bag</button>
              </div>
            </div>
          ))}
        </div>
...
const CareersModal = ({ onClose }: { onClose: () => void }) => {
  const jobs = [
    { title: 'Junior Barista', location: 'Marylebone, London', type: 'Full Time', desc: "We're looking for a passionate, curious Junior Barista to join our Marylebone team. You'll learn the craft of specialty coffee from our head baristas, develop your palate, and become part of a tight-knit team that genuinely cares about every cup.", reqs: ['A love of coffee and hospitality', 'Positive energy and team spirit', 'Flexibility across weekday and weekend shifts', 'Experience helpful but not essential — we train!'] },
    { title: 'Pastry Assistant', location: 'Swansea', type: 'Part Time', desc: "Join our Swansea kitchen team and help craft the seasonal bakes that our customers love. Early mornings, warm ovens, and the smell of cinnamon — this one's for the bakers.", reqs: ['Basic baking knowledge', 'Early morning availability', 'Attention to detail', 'Food hygiene certificate (or willingness to obtain)'] },
    { title: 'Front of House Lead', location: 'Paris', type: 'Full Time', desc: "Our Paris location is growing and we need a confident, warm Front of House Lead to run the floor. Bilingual (English/French) preferred. You'll be the face of Heirloom in the Marais.", reqs: ['Previous hospitality leadership experience', 'English and French speaker preferred', 'Calm under pressure', 'Passion for exceptional guest experience'] },
  ];
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          {jobs.map((job) => (
            <div key={job.title} className="bg-brand-cream-light rounded-2xl p-6">
              ...
              <ul className="space-y-1 mb-4">
                {job.reqs.map((r) => (
                  <li key={r} className="text-xs text-brand-brown/60 flex gap-2"><span className="text-brand-orange">→</span> {r}</li>
                ))}
              </ul>
              <button className="bg-brand-brown text-white px-6 py-2 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-black transition">Apply Now</button>
            </div>
          ))}
        </div>
      </motion.div>
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
      ...
      <div className="space-y-8">
        {[
          { source: 'The Guardian', date: 'March 2024', headline: '"Heirloom Coffee Is Quietly Becoming the UK\'s Most Loved Independent Café"', body: 'In a landscape dominated by global chains, Heirloom Coffee has done something remarkable — it has made people slow down. With its warm interiors, seasonally rotating menus, and obsessive attention to sourcing, the brand has cultivated a loyal following that crosses generations. "We never set out to be a chain," says founder James Whitmore. "We just wanted to make the best possible cup of coffee, every single day."' },
          { source: 'Eater London', date: 'January 2024', headline: '"The Best Autumn Menu in London Right Now"', body: "Heirloom's autumnal offering is nothing short of stunning. From the caramel apple tart — flaky, buttery, unapologetically indulgent — to the maple pecan latte that has become something of a cult obsession, every item on the menu feels considered and intentional. This is seasonal cooking done properly." },
          { source: 'Condé Nast Traveller', date: 'November 2023', headline: '"Paris Has Fallen for a British Coffee Brand — and We Understand Why"', body: 'When Heirloom Coffee opened its doors on Rue de Bretagne, the neighbourhood was sceptical. Six weeks later, there was a queue down the street. The Parisians, notoriously protective of their café culture, have embraced this British import wholeheartedly — a testament to the quality that Heirloom brings to every location it calls home.' },
        ].map((a) => (
          <div key={a.source} className="border-b border-brand-brown/10 pb-8 last:border-0 last:pb-0">
            <div className="flex items-center gap-3 mb-3">
              <span className="text-[10px] uppercase tracking-widest font-bold text-brand-orange">{a.source}</span>
              <span className="text-brand-brown/30 text-xs">· {a.date}</span>
            </div>
...
const SustainabilityModal = ({ onClose }: { onClose: () => void }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
      ...
      <h2 className="text-3xl font-serif font-black italic text-brand-brown mb-3">Our Commitment to the Planet</h2>
      <p className="text-brand-brown/60 text-sm mb-8 leading-relaxed">We believe great coffee and good values go hand in hand.</p>
      <div className="space-y-4">
        {[
          { icon: '🌱', title: 'Direct Trade Sourcing', desc: 'Every bean we use is sourced directly from smallholder farms we have visited and vetted personally. We pay above Fairtrade prices and build long-term relationships with our farmers.' },
          { icon: '♻️', title: 'Zero Single-Use Plastic', desc: 'Since 2019, we have eliminated single-use plastic across all three locations. Our cups are compostable, our lids are plant-based, and we offer a 20p discount to anyone who brings their own cup.' },
          { icon: '🍂', title: 'Seasonal & Local Baking', desc: 'Our bakery menu changes with the seasons, using locally grown produce wherever possible. This reduces food miles, supports British farmers, and means our menu is always at its freshest.' },
          ...
          { icon: '🤝', title: 'Community Partnerships', desc: 'We partner with local food banks and community kitchens to donate unsold food daily. In 2023 we donated over 4,000 items across our three stores.' },
          { icon: '📦', title: 'Plastic-Free Packaging', desc: 'All retail packaging is either recycled, recyclable, or compostable. We are working toward fully home-compostable packaging by 2026.' },
        ].map((item) => (
...
const PrivacyModal = ({ onClose }: { onClose: () => void }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
      ...
      <div className="flex items-center gap-2 mb-4">
        <Leaf size={16} className="text-brand-orange" />
        <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">Privacy</span>
      </div>
      ...
      <div className="space-y-6 text-sm text-brand-brown/70 leading-relaxed">
        {[
          { title: 'Who We Are', body: 'Heirloom Coffee Ltd is a specialty coffee company operating from London, Swansea, and Paris. We are committed to protecting your personal data and being transparent about how we use it.' },
          { title: 'What Data We Collect', body: 'We may collect your name, email address, delivery address, and payment information when you place an order or subscribe to our newsletter. We also collect anonymised usage data to improve our website experience.' },
          ...
          { title: 'Your Rights', body: 'Under UK GDPR, you have the right to access, correct, or delete your personal data at any time. You may also withdraw consent for marketing communications by clicking "unsubscribe" in any email we send.' },
          { title: 'Cookies', body: 'Our website uses essential cookies to function correctly and optional analytics cookies to understand how visitors use our site. You can manage cookie preferences at any time via your browser settings.' },
          { title: 'Data Retention', body: 'We retain personal data only for as long as necessary to fulfil the purpose it was collected for, or as required by law. Order data is held for 7 years in line with HMRC requirements.' },
          { title: 'Contact Us', body: 'If you have any questions about this policy or wish to exercise your rights, please contact our Data Protection Officer at privacy@heirloomcoffee.co.uk.' },
        ].map((s) => (
          <div key={s.title}>
            <h3 className="font-serif font-bold italic text-brand-brown text-lg mb-2">{s.title}</h3>
            <p>{s.body}</p>
          </div>
        ))}
      </div>
...
const OrderModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const [deliveryInfo, setDeliveryInfo] = useState({ name: '', address: '', time: '' });
  const [pickupInfo, setPickupInfo] = useState({ name: '', store: 'Marylebone', time: '' });
  const [orderNumber] = useState(() => Math.floor(100000 + Math.random() * 900000).toString());

  const addToBag = (item: typeof MENU_ITEMS[0]) => {
    setBag(prev => {
  ...
  const updateQty = (title: string, delta: number) => {
    setBag(prev => prev.map(b => b.title === title ? { ...b, qty: Math.max(0, b.qty + delta) } : b).filter(b => b.qty > 0));
  };
        ...
        {step === 'confirmed' ? (
          <div className="text-center py-8">
            ...
            <div className="flex items-center justify-center gap-2 mb-4">
              <Leaf size={16} className="text-brand-orange" />
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">Order Confirmed</span>
            </div>
            ...
            <p className="text-brand-brown/50 text-xs">Keep this number for your records. You'll receive a confirmation shortly.</p>
            <button onClick={onClose} className="mt-8 bg-brand-brown text-white px-8 py-3 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-black transition">
              Done
            </button>
          </div>
        ) : step === 'delivery' ? (
          <div>
            <button onClick={() => setStep('bag')} className="text-brand-orange text-xs mb-6 flex items-center gap-1">← Back</button>
            <div className="flex items-center gap-2 mb-2">
              <Truck size={16} className="text-brand-orange" />
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">Delivery</span>
            </div>
            <h2 className="text-3xl font-serif font-black italic text-brand-brown mb-6">Delivery Details</h2>
            <div className="space-y-4 mb-8">
              <div>
                <label className="text-xs uppercase tracking-widest text-brand-brown/50 font-bold">Full Name</label>
                <input value={deliveryInfo.name} onChange={e => setDeliveryInfo(p => ({ ...p, name: e.target.value }))}
                  placeholder="Jane Smith"
                  className="w-full border-b border-brand-brown/20 py-3 text-sm bg-transparent outline-none mt-1" />
              </div>
              <div>
                <label className="text-xs uppercase tracking-widest text-brand-brown/50 font-bold">Delivery Address</label>
                <input value={deliveryInfo.address} onChange={e => setDeliveryInfo(p => ({ ...p, address: e.target.value }))}
                  placeholder="12 Example Street, London"
                  className="w-full border-b border-brand-brown/20 py-3 text-sm bg-transparent outline-none mt-1" />
              </div>
              <div>
                <label className="text-xs uppercase tracking-widest text-brand-brown/50 font-bold">Preferred Time</label>
                <select value={deliveryInfo.time} onChange={e => setDeliveryInfo(p => ({ ...p, time: e.target.value }))}
                  className="w-full border-b border-brand-brown/20 py-3 text-sm bg-transparent outline-none mt-1">
            ...
            <div className="border-t border-brand-brown/10 pt-4 mb-6">
              ...
              <div className="flex justify-between text-sm mt-2">
                <span className="text-brand-brown/60">Delivery</span>
                <span className="font-bold text-brand-brown">£2.50</span>
              </div>
              <div className="flex justify-between text-sm mt-2 pt-2 border-t border-brand-brown/10">
                <span className="font-bold text-brand-brown">Total</span>
                <span className="font-bold text-brand-orange text-lg">£{(total + 2.5).toFixed(2)}</span>
              </div>
            </div>
            <button onClick={() => setStep('confirmed')}
              className="w-full bg-brand-orange text-white py-4 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-brand-brown transition">
              Pay £{(total + 2.5).toFixed(2)} → Confirm Order
            </button>
          </div>
        ) : step === 'pickup' ? (
          <div>
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">Store Pickup</span>
            </div>
            <h2 className="text-3xl font-serif font-black italic text-brand-brown mb-6">Pickup Details</h2>
            <div className="space-y-4 mb-8">
              <div>
                <label className="text-xs uppercase tracking-widest text-brand-brown/50 font-bold">Your Name</label>
                <input value={pickupInfo.name} onChange={e => setPickupInfo(p => ({ ...p, name: e.target.value }))}
                  placeholder="Jane Smith"
                  className="w-full border-b border-brand-brown/20 py-3 text-sm bg-transparent outline-none mt-1" />
              </div>
              <div>
                <label className="text-xs uppercase tracking-widest text-brand-brown/50 font-bold">Store</label>
                <select value={pickupInfo.store} onChange={e => setPickupInfo(p => ({ ...p, store: e.target.value }))}
                  className="w-full border-b border-brand-brown/20 py-3 text-sm bg-transparent outline-none mt-1">
                  <option>Marylebone, London</option>
                  <option>Swansea</option>
                  <option>Paris</option>
                </select>
              </div>
              <div>
                <label className="text-xs uppercase tracking-widest text-brand-brown/50 font-bold">Pickup Time</label>
                <select value={pickupInfo.time} onChange={e => setPickupInfo(p => ({ ...p, time: e.target.value }))}
                  className="w-full border-b border-brand-brown/20 py-3 text-sm bg-transparent outline-none mt-1">
          ...
          <div>
            <button onClick={() => setStep('menu')} className="text-brand-orange text-xs mb-6 flex items-center gap-1">← Back to Menu</button>
            <div className="flex items-center gap-2 mb-2">
              <ShoppingBag size={16} className="text-brand-orange" />
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">Your Bag</span>
            </div>
            <h2 className="text-3xl font-serif font-black italic text-brand-brown mb-6">Review Order</h2>
            {bag.length === 0 ? (
              <p className="text-brand-brown/50 text-sm">Your bag is empty.</p>
            ) : (
              <>
                <div className="space-y-4 mb-8">
                  {bag.map((b) => (
                    <div key={b.title} className="flex items-center justify-between bg-brand-cream-light rounded-2xl px-5 py-4">
                      <div>
                        <p className="font-serif font-bold italic text-brand-brown text-sm">{b.title}</p>
                        <p className="text-brand-brown/50 text-xs">£{b.price.toFixed(2)} each</p>
                      </div>
                ...
                <div className="border-t border-brand-brown/10 pt-4 mb-8">
                  <div className="flex justify-between">
                    <span className="font-bold text-brand-brown">Total</span>
                    <span className="font-bold text-brand-orange text-xl">£{total.toFixed(2)}</span>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => setStep('pickup')}
                    className="flex items-center justify-center gap-2 border-2 border-brand-brown text-brand-brown py-4 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-brand-brown hover:text-white transition">
                    <Store size={14} /> Store Pickup
                  </button>
                  <button onClick={() => setStep('delivery')}
                    className="flex items-center justify-center gap-2 bg-brand-orange text-white py-4 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-brand-brown transition">
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <Leaf size={16} className="text-brand-orange" />
                <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">Order Online</span>
              </div>
              {totalItems > 0 && (
                <button onClick={() => setStep('bag')}
                  className="flex items-center gap-2 bg-brand-brown text-white px-5 py-2 rounded-full text-xs font-bold hover:bg-black transition">
                  <ShoppingBag size={14} /> Bag ({totalItems}) · £{total.toFixed(2)}
                </button>
              )}
            </div>
            <h2 className="text-3xl font-serif font-black italic text-brand-brown mb-8">What would you like?</h2>
                ...
                return (
                  <div key={item.title} className="flex items-center gap-4 bg-brand-cream-light rounded-2xl px-5 py-4">
                    ...
                    <div className="flex-grow min-w-0">
                      <p className="font-serif font-bold italic text-brand-brown text-sm">{item.title}</p>
                      <p className="text-brand-brown/50 text-xs truncate">{item.description}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {bagItem ? (
                        ...
                        <button onClick={() => addToBag(item)}
                          className="w-7 h-7 rounded-full bg-brand-orange text-white flex items-center justify-center hover:bg-brand-brown transition">
                          <Plus size={14} />
                        </button>
                      )}
                    </div>
            ...
            {totalItems > 0 && (
              <button onClick={() => setStep('bag')}
                className="w-full mt-6 bg-brand-orange text-white py-4 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-brand-brown transition">
                View Bag ({totalItems}) · £{total.toFixed(2)}
              </button>
            )}
          </div>
        )}
    ...
    <div className="flex-shrink-0 w-72 md:w-80" style={{ height: '520px', perspective: '1200px' }}>
      <div className="relative w-full h-full" style={{ transformStyle: 'preserve-3d', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)', transition: 'transform 0.8s cubic-bezier(0.4, 0.2, 0.2, 1)' }}>
        <div className="absolute inset-0 bg-brand-cream-light rounded-[2rem] overflow-hidden shadow-sm border border-brand-brown/5 flex flex-col" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}>
          ...
          <div className="p-8 flex flex-col flex-grow overflow-hidden">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">{category}</span>
            </div>
        ...
        <div className="absolute inset-0 bg-white rounded-[2rem] shadow-2xl p-8 flex flex-col justify-between" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
          ...
          <button onClick={() => setFlipped(false)} className="mt-6 bg-brand-brown text-white px-6 py-3 rounded-full text-xs uppercase tracking-widest font-bold hover:bg-black transition">
            Back
          </button>
        </div>
      </div>
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 h-20 px-6 md:px-12 flex justify-between items-center backdrop-blur-md bg-black/20">
        ...
        <button onClick={onOrderOnline} className="hidden md:block bg-brand-brown border border-white text-white px-8 py-3 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-white hover:text-brand-brown transition">
          Order Online
        </button>
...
const Footer = ({ onShowStory, onShowLocations, onShowShop, onShowCareers, onShowPress, onShowSustainability, onShowPrivacy }: {
  onShowStory: () => void; onShowLocations: () => void; onShowShop: () => void;
  onShowCareers: () => void; onShowPress: () => void; onShowSustainability: () => void; onShowPrivacy: () => void;
}) => {
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-16">
        ...
        <div>
          ...
          <ul className="space-y-3 text-white/60 text-sm">
            ...
            <li><button onClick={onShowStory} className="hover:text-brand-orange transition">Story</button></li>
            <li><button onClick={onShowLocations} className="hover:text-brand-orange transition">Locations</button></li>
        ...
        <div>
          ...
          <button onClick={handleSubscribe} className="mt-4 text-[10px] uppercase tracking-widest font-bold text-brand-orange hover:text-white transition">
            Subscribe →
          </button>
        </div>
      </div>
  ...
  return (
    <div className="bg-brand-cream min-h-screen overflow-x-hidden">
      ...
      <section className="relative h-screen flex items-center">
        <img src="https://plus.unsplash.com/premium_photo-1665827719939-3975a75b0062?q=80&w=1170&auto=format&fit=crop"
          className="absolute inset-0 w-full h-full object-cover" alt="Cafe" loading="eager" />
      ...
      <AnimatePresence>{showStory && <OurStoryModal onClose={() => setShowStory(false)} />}</AnimatePresence>
