...
const OurStoryModal = ({ onClose }: { onClose: () => void }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }} onClick={onClose}>
    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} onClick={e => e.stopPropagation()} className="bg-brand-cream-light rounded-[2rem] p-12 max-w-lg w-full relative shadow-2xl">
      ...
      <div className="flex items-center gap-2 mb-6"><Leaf size={16} className="text-brand-orange" /><span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">Our Story</span></div>
    ...
    <>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.85)' }} onClick={onClose}>
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} onClick={e => e.stopPropagation()} className="bg-brand-cream rounded-[2rem] p-8 w-full max-w-2xl relative shadow-2xl overflow-y-auto max-h-[90vh]">
          ...
          {totalItems > 0 && (
            <button onClick={() => setShowCheckout(true)} className="w-full bg-brand-orange text-white py-4 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-brand-brown transition">
              Checkout ({totalItems} items) · £{total.toFixed(2)}
            </button>
          )}
        </motion.div>
      </motion.div>
      <AnimatePresence>
        {showCheckout && <ShopCheckoutModal bag={bag} onClose={() => setShowCheckout(false)} />}
      </AnimatePresence>
    </>
  );
};
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} onClick={e => e.stopPropagation()} className="bg-brand-cream rounded-[2rem] p-8 w-full max-w-2xl relative shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        {submitted ? (
          <div className="text-center py-12">
            ...
            <h2 className="text-4xl font-serif font-black italic text-brand-brown mb-4">Thank You!</h2>
            <p className="text-brand-brown/60 text-sm leading-relaxed mb-2">We've received your application for <strong className="text-brand-brown">{applyingFor}</strong>.</p>
            <p className="text-brand-brown/60 text-sm">We'll be in touch soon. In the meantime, come say hello over a coffee.</p>
          ...
          <div>
            ...
            <div className="space-y-6 mb-8">
              <div>
                <label className="text-xs uppercase tracking-widest text-brand-brown/50 font-bold">Email Address *</label>
                <input value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="jane@example.com" className="w-full border-b border-brand-brown/20 py-3 text-sm bg-transparent outline-none mt-1" />
              </div>
              <div>
                ...
                <textarea
                  ...
                  placeholder="In 250 words, tell us why you'd be a great addition to the Heirloom team..."
                  rows={8}
                  className="w-full border border-brand-brown/20 rounded-2xl p-4 text-sm bg-brand-cream-light outline-none resize-none leading-relaxed mt-1"
                />
              </div>
            </div>
            <button onClick={handleSubmit} className="w-full bg-brand-orange text-white py-4 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-brand-brown transition">
              Send Application →
            </button>
          </div>
        ) : (
...
const SustainabilityModal = ({ onClose }: { onClose: () => void }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.85)' }} onClick={onClose}>
    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} onClick={e => e.stopPropagation()} className="bg-brand-cream rounded-[2rem] p-8 w-full max-w-2xl relative shadow-2xl overflow-y-auto max-h-[90vh]">
      <button onClick={onClose} className="absolute top-6 right-6 text-brand-brown hover:text-brand-orange transition"><X size={24} /></button>
      <div className="flex items-center gap-2 mb-4"><Leaf size={16} className="text-brand-orange" /><span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-orange">Sustainability</span></div>
      ...
      <div className="space-y-4">
        {[
          { icon: '🌱', title: 'Direct Trade Sourcing', desc: 'Every bean we use is sourced directly from smallholder farms we have visited personally. We pay above Fairtrade prices and build long-term relationships with our farmers.' },
          { icon: '♻️', title: 'Zero Single-Use Plastic', desc: 'Since 2019, we have eliminated single-use plastic across all three locations. Our cups are compostable and we offer a 20p discount to anyone who brings their own cup.' },
          ...
          <div key={item.title} className="flex gap-4 p-5 bg-brand-cream-light rounded-2xl">
            ...
            <div><h3 className="font-serif font-bold italic text-brand-brown text-lg mb-1">{item.title}</h3><p className="text-brand-brown/70 text-sm leading-relaxed">{item.desc}</p></div>
          </div>
        ))}
      </div>
    </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} onClick={e => e.stopPropagation()} className="bg-brand-cream rounded-[2rem] p-8 w-full max-w-2xl relative shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        {step === 'confirmed' ? (
          <div className="text-center py-8">
            ...
            <p className="text-brand-brown/50 text-xs">Keep this number for your records.</p>
            <button onClick={onClose} className="mt-8 bg-brand-brown text-white px-8 py-3 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-black transition">Done</button>
          </div>
        ) : step === 'delivery' ? (
          <div>
          ...
          <div>
            ...
            {totalItems > 0 && (
              <button onClick={() => setStep('bag')} className="w-full mt-6 bg-brand-orange text-white py-4 rounded-full text-[10px] uppercase tracking-widest font-bold hover:bg-brand-brown transition">
    ...
    <footer className="bg-[#1a0f0a] text-white pt-32 pb-16 px-6 md:px-12">
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-16">
        ...
        <div>
          ...
          <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" className="w-full bg-transparent border-b border-white/20 py-3 text-sm outline-none" onKeyDown={e => e.key === 'Enter' && handleSubscribe()} />
          <button onClick={handleSubscribe} className="mt-4 text-[10px] uppercase tracking-widest font-bold text-brand-orange hover:text-white transition">Subscribe →</button>
        </div>
      </div>
